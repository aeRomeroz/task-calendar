...
export const ContinuousCalendar: React.FC<ContinuousCalendarProps> = ({ onClick }) => {
  ...
  async function fetchMonth(year: number, month: number) {
    try {
      ...
      const data: DayActivity[] = await res.json(); // tipado opcional
      return data;
    } catch (err) {
      console.error(err);
      return [];
  ...
  async function addActivity(day: number, month: number, year: number, trainingDone: boolean, description?: string) {
    await fetch('http://localhost:5291/api/calendar', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ day, month, year, trainingDone, eventDescription: description })
    });
  }

  const handleToggleTraining = (day: number, month: number, year: number) => {
    const currentState = activeDays[`${month}-${day}`] ?? false;
    addActivity(day, month, year, !currentState); // invierte el estado
        ...
        {week.map(({ month, day }, dayIndex) => {
          ...
          return (
            ...
            >
              ...
              <div className="absolute left-2 top-8 w-full h-hug truncate rounded-3xl bg-slate-800 py-0.5 px-2.5 border border-transparent text-sm text-white transition-all shadow-sm sm:bottom-0 sm:text-lg lg:bottom-2.5 lg:left-3.5 lg:-mb-1 lg:w-fit lg:px-0 lg:text-xl 2xl:mb-[-4px] 2xl:text-2xl">
                Hola
              </div>
              {isNewMonth && (
